...
begin
  key.FNumber:='';
  key.FSeria:='';
  if QDialogs.InputQuery('Удаление','Введите номер паспорта', key.FNumber)
    then
      if HashTable.Delete(key)
        then ShowMessage('Пользователь удален')
        else MessageDlg('Пользователь не найден',mtError,[mbCancel],0)
end;
    ...
    try
      if ShowModal=mrOK then
          if not HashTable.Add(Info)
            then
              begin
                MessageDlg('Пользователь с таким именем уже присутствует в таблице',mtError,[mbCancel],0);
                Info.Free
              end;
    finally
      Free
...
begin
  Key.FNumber:='';
  key.FSeria:='';
  if InputQuery('Поиск', 'Введите номер паспорта', Key.FNumber) then
    begin
      if HashTable.Find(Key, MyInfo)
        then
          begin
            ...
            with FormShow do
            begin
              Caption:='Найденный пользователь';
              Edit_FIO.Text:=MyInfo.FIO;
              Adres.Text:=MyInfo.Adres;
              Edit_PaspSeria.Text:=MyInfo.key.FSeria;
              PaspNumber.Text:= MyInfo.key.FNumber;
              Edit_FIO.ReadOnly:=True;
              ADres.ReadOnly:=True;
              Edit_PaspSeria.ReadOnly:=True;
